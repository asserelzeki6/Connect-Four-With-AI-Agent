...
def minimize(state, depth, max_depth, alpha, beta):
    ...
    for child_board in get_children(state, 'x'):
        child_node, _ = maximize(child_board, depth + 1, max_depth, alpha, beta)
...
def maximize(state, depth, max_depth, alpha, beta):
    ...
    for child_board in get_children(state, 'r'):
        child_node, _ = minimize(child_board, depth + 1, max_depth, alpha, beta)
        current_node.add_child(child_node)
...
def decision(state):
    root_node, best_move = maximize(state, 0, 3, float('-inf'), float('inf'))
